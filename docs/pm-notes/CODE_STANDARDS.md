# COFACTOR ADMIN - CODE STANDARDS

**Version 1.0 | February 2026**



**Status:** Draft - pending sign-off by Theis before feature work begins.
...
```typescript
async function approveOperatorApplication(user: User) {
  const application = await findPendingOperatorApplication(user.id);
  validateApplicationEligibility(application);
  await promoteUserToOperator(user.id);
...
```typescript
async function createArticlePage(data: FormData) {
...
async function createArticlePage(data: FormData) {
  const pageData = parseArticlePageForm(data);
  await validateUniqueSlug(pageData.slug);
  const sanitizedContent = sanitizeHtmlContent(pageData.content);
  return await saveArticlePage({ ...pageData, content: sanitizedContent });
}
...
async function saveArticlePage(data: ArticlePageData) {
  /* ... */
}
```
...
```typescript
function calculateTrustScore(user: User) {
  let score = 0;

  switch (user.role) {
    case "MEMBER":
      score = user.invites * 50;
      break;
    case "OPERATOR":
      score = user.invites * 50 + user.approvedEdits * 20;
      break;
    case "ADMIN":
...
const POWER_SCORE_CALCULATORS = {
  MEMBER: (user: User) => user.invites * 50,
  OPERATOR: (user: User) => user.invites * 50 + user.approvedEdits * 20,
  ADMIN: () => 1000,
};
...
async function setAndReturnPublished(pageId: string): Promise<boolean> {
  await prisma.knowledgeBasePage.update({
    where: { id: pageId },
    data: { published: true },
  });
  return true;
...
async function isPagePublished(pageId: string): Promise<boolean> {
  const page = await prisma.knowledgeBasePage.findUnique({ where: { id: pageId } });
...
function calculateUserTrustScore(user: User) {
  /* ... */
}
```
...
-  `calculateTrustScore.ts` - Only changes if scoring logic changes
...
| **Server Actions**   | camelCase + `.actions.ts`   | `article.actions.ts`              |
...
export async function saveArticleContent(content: string) {
...
model ArticleRevision {
  knowledgeBasePageId String
  authorId  String

  // Always index foreign keys
  @@index([knowledgeBasePageId])
  @@index([authorId])
}
```
...
```typescript
async function approveArticleRevision(revisionId: string) {
  await prisma.$transaction(async (tx) => {
    // Step 1: Update revision status
    const revision = await tx.articleRevision.update({
      where: { id: revisionId },
      data: { status: "APPROVED" },
    });

    // Step 2: Update page content
    await tx.knowledgeBasePage.update({
      where: { id: revision.knowledgeBasePageId },
      data: {
...
4. Business logic (trust score, approval workflows)
...
```typescript
// lib/utils/calculateTrustScore.ts
export function calculateTrustScore(user: User): number {
  const invitePoints = user.inviteCount * 50;
  const articlePoints = user.approvedEdits * 20;
  const socialPoints = Math.floor(user.totalFollowers / 100);

  return invitePoints + articlePoints + socialPoints;
}

// tests/unit/calculateTrustScore.test.ts
describe("calculateTrustScore", () => {
  it("calculates score with all factors", () => {
    ...
    expect(calculateTrustScore(user)).toBe(320);
  ...
  it("handles zero values", () => {
    ...
    expect(calculateTrustScore(user)).toBe(0);
  });
});
```
...
```typescript
// tests/integration/article.actions.test.ts
describe("proposeArticleEdit", () => {
  it("creates pending revision for member", async () => {
    const member = await createTestUser({ role: "MEMBER" });
    const page = await createTestArticlePage();

    await proposeArticleEdit({
      pageId: page.id,
      content: "New content",
      userId: member.id,
    });
  ...
  it("auto-approves for operator", async () => {
...
1. User signup with invite code
2. Email verification
3. Create article page
4. Propose article edit (member)  Admin approval
5. Link SSO account
