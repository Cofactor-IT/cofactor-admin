...
import { requestPasswordReset, type ForgotPasswordActionState } from "../../../actions/password-reset.actions"
    ...
    <main className="admin-shell">
      <section className="admin-page-content admin-auth-content">
        <form action={formAction} className="admin-card admin-auth-form admin-auth-form-wide">
          ...
          <p className="body m-0">Enter your @cofactor.world email and we will send reset instructions.</p>

          <label className={`label${hasFieldError(state, "email") ? " admin-field-error" : ""}`} htmlFor="email">
            Email
          </label>
          <input
            id="email"
            ...
            required
          />
          {firstFieldError(state, "email") ? <p className="caption admin-field-error">{firstFieldError(state, "email")}</p> : null}

          {state.message ? <p className={`caption${state.success ? "" : " admin-field-error"}`}>{state.message}</p> : null}

          {state.devResetUrl ? (
            <p className="caption">
              Dev reset link: <a className="admin-text-button" href={state.devResetUrl}>open reset page</a>
            </p>
          ) : null}
