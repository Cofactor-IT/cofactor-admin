...
import { findGreetingSignalMetrics, type GreetingSignalMetrics } from "../database/queries/greeting"
...
function overdueNextStepsSignal(count: number): GreetingSignal | null {
  ...
  return { message: `You have ${count} overdue ${pluralize(count, "next step")} in your CRM.`, priority: 1 }
}

function staleSubmissionsSignal(count: number): GreetingSignal | null {
  if (count === 0) return null
  const verb = count === 1 ? "has" : "have"
  return { message: `${count} ${pluralize(count, "submission")} ${verb} been in review for over 48 hours.`, priority: 2 }
}

function newSubmissionsSignal(count: number): GreetingSignal | null {
  if (count === 0) return null
  return { message: `${count} new ${pluralize(count, "submission")} since your last visit.`, priority: 3 }
}

function queuedSubmissionsSignal(count: number): GreetingSignal | null {
  if (count === 0) return null
  return { message: `${count} ${pluralize(count, "submission")} waiting for review.`, priority: 4 }
}
