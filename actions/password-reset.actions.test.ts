...
describe("requestPasswordReset", () => {
  ...
  it("rejects invalid email before DB queries", async () => {
    const state = await requestPasswordReset({ success: false, message: "" }, buildForgotFormData("wrong@gmail.com"))
  ...
  it("issues token for active account and logs audit", async () => {
    ...
    vi.mocked(buildPasswordResetUrl).mockReturnValue("http://localhost:3000/auth/reset-password?token=raw-token")
